...
void app_main() {
    ...
    while (1) {
        ...
        if (err == ESP_OK) {
          ...
          int voltage_manual_mv = (adc_raw * ADC_VMAX_MV) / ADC_MAX_RAW;

          if (voltage_manual_mv > 0) {
            error_value = ((float)abs(voltage_mv - voltage_manual_mv) / voltage_manual_mv) * 100.0f;
          } else {
            error_value = 0.0f;
          }

          printf("%d\t%d\t\t%d\t%.2f%%\n", adc_raw, voltage_manual_mv, voltage_mv, error_value);
        }
